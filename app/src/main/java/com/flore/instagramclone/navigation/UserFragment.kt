...
class UserFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (uid == currentUserUid) { // 자신의 uid와 가져온 uid 값이 일치하면 자신 페이지로 이동
            fragmentView?.btn_account_follow_signout?.text =
                getString(R.string.signout) // 자신 페이지의 경우 버튼을 "Signout"
            fragmentView?.btn_account_follow_signout?.setOnClickListener {
                ...
                ) // 현재 엑티비티를 종료하고 로그인 엑티비티로 이동
                auth?.signOut() // auth의 인자값에 있는 uid를 로그아웃 요청
            }
        } else { // 다른 경우 상대방의 유저페이지 나타내기
            fragmentView?.btn_account_follow_signout?.text =
                getString(R.string.follow) // 상대 페이지의 경우 버튼을 "follow"
            var mainactivity = (activity as MainActivity)
            mainactivity?.toolbar_username?.text =
                arguments?.getString("userId") // 로그인한 유저 id를 가져와서 텍스트 세팅
            mainactivity?.btn_toolbar_back?.setOnClickListener {
                mainactivity.bottom_navigation.selectedItemId =
                    R.id.home_menu // 뒤로가기 버튼을 누르면 홈 메뉴로 이동하는 이벤트로 설정
            }
